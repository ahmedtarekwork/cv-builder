...
const AuthForm = ({ type }: Props) => {
  ...
  return (
    <form
      className="bg-white rounded-[15px] mt-4 mx-auto the-shadow p-4 space-y-4 w-[500px] max-w-full [&>*]:max-w-screen"
    ...
    >
      ...
      {type === "signup" && (
        <>
          <Input placeholder="name" type="text" ref={InputsRefs.current.name} />
          {errors.name && <p className="text-destructive">{errors.name}</p>}
        </>
      )}

      <Input placeholder="email" type="email" ref={InputsRefs.current.email} />
      ...
      <Button className="w-full capitalize" type="submit" disabled={isLoading}>
